...
// Format80: LZ copy commands against the output produced so far.
void lcwDecode(const std::vector<uint8_t>& src, size_t srcOffset, std::vector<uint8_t>& dest) {
    ...
    auto checkSpace = [&](size_t count) {
        if (destIndex + count > dest.size())
            throw std::runtime_error("SHP: LCW output overflow");
    };
    ...
    while (true) {
        ...
        if ((i & 0x80) == 0) {
            ...
            if (rpos > destIndex)
                throw std::runtime_error("SHP: LCW copy before start of output");
            ...
            if (count3 == 0x3e) {
                ...
                if (srcIndex >= destIndex)
                    throw std::runtime_error("SHP: LCW forward reference");
                checkSpace(count);
                for (size_t n = 0; n < count; n++)
